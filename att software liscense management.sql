...
/* D  ex pesquisado no google senti um pouco de dificuldade*/
...
/* G acredito que 1 byte seja um conjunto de 8 caracteres nesse caso*/

select *, octet_length(Nome_RazaoSocial) as numero_byte from lcliente order by Nome_RazaoSocial asc;

/*  H  pesquisei no google porque senti um pouco de dificuldade */
...
/*  W   */
 ...
 select sum(ValorAquisicao) as valor_licenca, NomeSoftware 
 from llicenca as lc
 inner join lversao as vs on vs.Versao = lc.Versao_FK
 inner join lsoftware as sf on sf.idSOFTWARE = vs.idSOFTWARE_FK
...
select cl.Nome_RazaoSocial, count(lc.NumLicenca) as qnt_licenca, 
 sf.NomeSoftware, tp.DescricaoTipo
from llicenca as lc
inner join lcliente as cl on cl.idCLIENTE = lc.idCLIENTE_FK
inner join ltipo_empresa as tp on tp.idTIPO_Empresa = cl.idTIPO_Empresa_FK
inner join lversao as vs on vs.Versao = lc.Versao_FK
inner join lsoftware as sf on sf.idSOFTWARE = vs.idSOFTWARE_FK
group by sf.NomeSoftware, cl.Nome_RazaoSocial, tp.DescricaoTipo
order by cl.Nome_RazaoSocial, sf.NomeSoftware  asc;
